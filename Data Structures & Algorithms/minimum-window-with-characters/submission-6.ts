class Solution {
    ...
    minWindow(s: string, t: string): string {
        ...
        let i=start;
        for(;(i<s.length && currentMatchCount!== matchCount);i++) {
            if(tMap.has(s[i])) {
                let newVal = (sMap.get(s[i]) ?? 0)+1
                sMap.set(s[i], newVal)
                if (newVal == tMap.get(s[i])) {
                    console.log(`increasing match count for s[${i}] - ${s[i]}`)
                    currentMatchCount++;
                }
            }
        }
        ...
        while(currentMatchCount == matchCount) {
            if(shortestStr == "" || shortestStr.length > i-start) {
                    shortestStr = s.slice(start, i)
            }
